...
class Inventory(QWidget):
    ...
    def set_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(["Name", "Quantity", "Price", "Total"])
        ...
        self.out_btn=QPushButton("Log Out")
        self.out_btn.setObjectName("logout")
        self.out_btn.clicked.connect(self.log_out)
    ...
    def load_item(self):
        ...
        cur = conn.cursor()
        cur.execute("SELECT name,quantity,price,total FROM items WHERE userid=?",(self.user_id,))
        r = cur.fetchall()
        conn.close()
    ...
    def add_item(self):
        ...
        conn = sqlite3.connect('inventory.db')
        cur = conn.cursor()
        cur.execute("INSERT INTO items (name,quantity,price,total,userid) VALUES (?,?,?,?,?)", (n, q, p, t,self.user_id))
        conn.commit()
        conn.close()
    ...
    def delete_item(self):
        ...
        conn = sqlite3.connect('inventory.db')
        cur = conn.cursor()
        cur.execute('DELETE FROM items WHERE sn=? and userid=?', (item_id,self.user_id))
        conn.commit()
        conn.close()
        self.load_item()
    ...
    def update_item(self):
        ...
        conn = sqlite3.connect('inventory.db')
        cur = conn.cursor()
        cur.execute("UPDATE items SET name=?,quantity=?,price=?,total=? WHERE sn=? and userid=?", (n, q, p, t, item_id,self.user_id))
        conn.commit()
        conn.close()
        self.load_item()
